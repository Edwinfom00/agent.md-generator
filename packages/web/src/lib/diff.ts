...
/**
 * Groups diff lines into hunks showing only changes and context around them.
 */
    ...
    if (isChange) {
      ...
      while (k >= 0 && backCount < contextSize) {
        if (diffLines[k].type === 'unchanged') {
          ...
          break // Stop if another change is nearby, it will be handled or merged
        }
        k--
      }

      // If we have an existing hunk and our context overlaps or touches it, merge them
      if (hunks.length > 0) {
        ...
        if (firstOldNo - lastOldNo <= contextSize * 2) {
          // Merge! Pop it so we can expand it
          hunks.pop()
          // Re-fill intermediate unchanged lines
      ...
      // Add the current change line
      currentHunkLines.push(line)

      // Gather changes and trailing context
      i++
      ...
      while (i < total) {
        ...
        if (isNextChange) {
          ...
          unchangedStreak++
          currentHunkLines.push(nextLine)
          if (unchangedStreak >= contextSize) {
            break
          }
        }
        i++
      ...
      const lastLine = currentHunkLines[currentHunkLines.length - 1]
